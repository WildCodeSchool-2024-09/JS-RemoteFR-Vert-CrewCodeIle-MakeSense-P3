...
export default function Vote({ id }: VoteProps) {
    ...
    const checkUserVote = async () => {
      try {
        ...
        if (response.ok) {
          ...
          console.log("existingvoterecu", existingVote);
  ...
  const submitVote = async (state: boolean) => {
    ...
    try {
      ...
      let url: string;
      let method: string;
      //ajout ici faut il l'ajouter au commentaire
      if (voteId === null || Number.isNaN(voteId)) {
      ...
      console.log("token envoyé", auth.token);
